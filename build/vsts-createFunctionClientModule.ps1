...
$parentModule = 'FortigateManager'
if (-not $ModuleName) { $ModuleName = 'FortigateManager.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\FortigateManager'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\FortigateManager.psd1"
$prereqHash = @{
